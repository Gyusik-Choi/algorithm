...
// 인접행렬을 하고
// Math.min 으로 비교해서
// 도착 정점의 값에 가장 작은 값 하나만 들어가게끔 처리하려고 했으나
// 입력량이 많은데 매번 대소 비교를 수행하다보니 메모리 초과 발생

for (let i = 0; i < N - 1; i++) {
  const xStart = xPlanets[i][1];
  const xEnd = xPlanets[i + 1][1];
  const xStartDist = xPlanets[i][0];
  const xEndDist = xPlanets[i + 1][0];

  planets[xStart][xEnd] = Math.min(planets[xStart][xEnd], xEndDist - xStartDist);
  planets[xEnd][xStart] = Math.min(planets[xEnd][xStart], xEndDist - xStartDist);

  const yStart = yPlanets[i][1];
  const yEnd = yPlanets[i + 1][1];
  const yStartDist = yPlanets[i][0];
  const yEndDist = yPlanets[i + 1][0];

  planets[yStart][yEnd] = Math.min(planets[yStart][yEnd], yEndDist - yStartDist);
  planets[yEnd][yStart] = Math.min(planets[yEnd][yStart], yEndDist - yStartDist);

  const zStart = zPlanets[i][1];
  const zEnd = zPlanets[i + 1][1];
  const zStartDist = zPlanets[i][0];
  const zEndDist = zPlanets[i + 1][0];

  planets[zStart][zEnd] = Math.min(planets[zStart][zEnd], zEndDist - zStartDist);
  planets[zEnd][zStart] = Math.min(planets[zEnd][zStart], zEndDist - zStartDist);
}
