...
def is_fish_possible_to_move(fish_way, y_idx, x_idx):
    ...
    while cnt_num < 7:
        ...
        if 0 <= y_idx < 4 and 0 <= x_idx < 4 and sea[y_idx][x_idx] == -1:
            return [y, x]
        else:
            cnt_num += 1
...
def get_possible_shark_move(s_y, s_x, visited):
    ...
    while True:
        ...
        if 0 <= y < 4 and 0 <= x < 4:
            possible_shark_move.append([y, x])
        else:
            break

    return possible_shark_move


def back_tracking_shark(s_y, s_x, visited):
    possible_shark_move = get_possible_shark_move(s_y, s_x, visited)
    fish_move(visited)
...
back_tracking_shark(0, 0, copied_sea)
