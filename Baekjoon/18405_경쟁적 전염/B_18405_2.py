...
def bfs(virus_location, h):
    y_value = [-1, 0, 1, 0]
    x_value = [0, 1, 0, -1]

    while h:
        sec, virus_num, y, x = heapq.heappop(h)
        ...
        for k in range(4):
            ...
            if 0 <= y_idx < N and 0 <= x_idx < N:
                if not virus_location[y_idx][x_idx]:
                    ...
                    heapq.heappush(h, (sec + 1, virus_num, y_idx, x_idx))
