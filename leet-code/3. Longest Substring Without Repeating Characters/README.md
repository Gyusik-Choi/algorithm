...
#### LongestSubstringWithoutRepeatingCharacters3_5

교재의 풀이와 다른 방식으로 풀이했다.

두 개의 해시맵을 두고 하나는 문자, 인덱스 조합(이하 문자 해시맵)으로 구성하고 다른 하나는 인덱스, 문자 조합(이하 인덱스 해시맵)으로 구성한다.

두 해시맵에는 현재 비교범위에 해당하는 값만 존재하도록 했다. 문자 해시맵에 중복된 문자가 있을 경우 중복된 문자보다 앞선 인덱스의 문자가 무엇인지 문자 해시맵 전체를 탐색하지 않고 바로 찾을 수 있도록 인덱스 해시맵을 사용한다.

해시맵에 넣거나 뺄때 두 해시맵을 모두 변경해야 한다.
