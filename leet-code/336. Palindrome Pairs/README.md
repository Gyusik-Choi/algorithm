...
#### PalindromePairs336_3

매우 어려운 문제였다. 몇 번의 수정을 통해 정확성은 통과했으나 2개 케이스의 시간 초과가 발생했다.

원인은 전혀 생각하지 못한 substring 에 있었고 Claude Code 의 도움으로 발견했다. 문자열의 현재 인덱스 뒤쪽의 문자열 부분이 팰린드롬인지 확인할때 문자열을 substring 으로 잘라서 인자로 넣었다. substring 은 문자 배열을 복사하기 때문에 긴 문자열에서는 비효율적이다. 그래서 substring 대신 인덱스 정보를 인자로 문자열과 함께 넘겨서 팰린드롬 여부를 판단하도록 변경해서 통과할 수 있었다. 
